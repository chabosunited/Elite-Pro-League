...
function renderPlayerProfile(data, slug){
  ...
  const p=data.profile, stats=data.stats||{}, posts=data.posts||[], matches=data.recentMatches||[], achievements=data.achievements||[];
  ...
  const matchCards=matches.length?matches.slice(0,3).map((m,i)=>`<article class="panel ref-match-card">
      <div class="ref-card-head"><span>${esc(i===0?'LETZTES SPIEL':'MATCH')}</span><small>${formatDateTime(m.scheduled_at)}</small></div>
      <div class="ref-score-row"><span>${esc(m.home_name)}</span><strong>${m.home_score ?? '-'} : ${m.away_score ?? '-'}</strong><span>${esc(m.away_name)}</span></div>
      <div class="ref-match-sub"><span><b>${fmt(m.goals)}</b>Tore</span><span><b>${fmt(m.assists)}</b>Assists</span><span><b>${Number(m.rating||0).toFixed(1)}</b>Rating</span></div>
    </article>`).join(''):`<article class="panel ref-empty-card">${emptyCard('Noch keine Matches','Sobald bestätigte Match-Statistiken vorliegen, erscheinen hier die letzten Spiele.')}</article>`;
  const postCards=posts.length?posts.map(post=>`<article class="panel ref-post"><div class="ref-post-head"><img src="${avatarFor(p)}"><div><strong>${esc(p.username)}</strong><small>${formatDateTime(post.created_at)}</small></div></div><p>${esc(post.body)}</p>${post.media_key?`<img class="ref-post-media" src="${assetUrl(post.media_key,'')}" alt="">`:''}<div class="ref-post-meta"><span>♥ ${fmt(post.likes)}</span><span>◌ ${fmt(post.comments)}</span></div></article>`).join(''):`<article class="panel ref-empty-card">${emptyCard('Noch keine Beiträge','Beiträge und Highlights des Spielers erscheinen hier automatisch.')}</article>`;
  const achievementCards=achievements.length?achievements.slice(0,4).map(a=>`<div class="ref-trophy"><div class="ref-trophy-icon">🏆</div><strong>${esc(a.title)}</strong><small>${esc(a.subtitle||'')}</small></div>`).join(''):`<div class="ref-mini-empty">Noch keine Trophäen</div>`;
  const attrs=[['PAC',p.pac],['SHO',p.sho],['PAS',p.pas],['DRI',p.dri],['DEF',p.def],['PHY',p.phy]];
  return `<section class="ref-player-hero">
    <img class="ref-player-cover" src="${coverFor(p)}" alt="Titelbild">
    <div class="container ref-player-hero-inner">
      <div class="ref-avatar"><img src="${avatarFor(p)}" alt="${esc(p.username)}"><span></span></div>
      <div class="ref-player-copy"><h1>${esc(p.username)} ${p.verified?'<i class="verified">✓</i>':''}</h1><div class="ref-subtitle">${esc(pos)} • ${esc(p.club||'Elite Pro League')}</div><p>${esc(p.bio||'Neu in der Elite Pro League.')}</p><div class="profile-actions"><button class="btn btn-primary" data-follow="${followKey}">${following?'✓ FOLGE ICH':'+ FOLGEN'}</button><button class="btn btn-ghost" data-message>▣ Nachricht</button>${own?'<button class="btn btn-ghost" data-edit-profile>✎ Profil bearbeiten</button>':''}</div></div>
    </div>
  </section>
  <div class="container ref-profile-wrap">
    ...
    <div class="ref-tabs">${['Beiträge','Highlights','Statistiken','Karriere','Clubs'].map((x,i)=>`<button class="tab-btn ${i===0?'active':''}">${x}</button>`).join('')}</div>
    <div class="ref-profile-grid"><section class="ref-feed-column">${postCards}</section><section class="ref-center-column">${matchCards}</section><aside class="ref-side-column">
      <section class="panel ref-side-box"><div class="ref-box-title"><span>SAISON STATISTIKEN</span></div><div class="ref-season-grid"><div><strong>${fmt(stats.matches)}</strong><span>Spiele</span></div><div><strong>${fmt(stats.goals)}</strong><span>Tore</span></div><div><strong>${fmt(stats.assists)}</strong><span>Assists</span></div><div><strong>${Number(stats.rating||0).toFixed(2)}</strong><span>Rating</span></div></div></section>
      <section class="panel ref-side-box"><div class="ref-box-title"><span>TROPHÄEN</span></div><div class="ref-trophy-grid">${achievementCards}</div></section>
      <section class="panel ref-side-box"><div class="ref-box-title"><span>ATTRIBUTE</span><b>GES ${fmt(p.overall)}</b></div><div class="ref-attrs">${attrs.map(([k,v])=>`<div><span>${k}</span><i><em style="width:${Math.max(0,Math.min(100,Number(v||0)))}%"></em></i><strong>${fmt(v)}</strong></div>`).join('')}</div></section>
    </aside></div>
  </div>`;
}
...
function renderClubProfile(data, slug){
  ...
  const postList=posts.length?posts.slice(0,3).map(p=>`<article class="ref-club-post"><img src="${clubLogoFor(c)}"><div><strong>${esc(c.name)}</strong><small>${formatDateTime(p.created_at)}</small><p>${esc(p.body)}</p><div class="ref-post-meta"><span>♥ ${fmt(p.likes)}</span><span>◌ ${fmt(p.comments)}</span></div></div>${p.media_key?`<img class="ref-club-post-media" src="${assetUrl(p.media_key,'')}">`:''}</article>`).join(''):`<div class="ref-mini-empty big">Noch keine Club-Beiträge.</div>`;
  const squadCards=squad.length?squad.slice(0,5).map(m=>`<a class="ref-squad-card" data-link href="/spieler/${esc(m.username.toLowerCase())}"><span class="ref-squad-rating">${fmt(m.overall)}</span><span class="ref-squad-pos">${esc(m.position||'--')}</span><img src="${avatarFor(m)}"><strong>${esc(m.username)}</strong><small>${esc(m.role||'PLAYER')}</small></a>`).join(''):`<div class="ref-mini-empty big">Noch kein Kader vorhanden.</div>`;
  ...
  <div class="container ref-club-wrap"><div class="ref-tabs">${['Feed','Kader','Statistiken','Spiele','Transfers','Galerie'].map((x,i)=>`<button class="tab-btn ${i===0?'active':''}">${x}</button>`).join('')}</div><div class="ref-club-grid">
    <section class="panel ref-club-feed"><div class="ref-box-title"><span>AKTUELLE BEITRÄGE</span></div>${postList}</section>
    <section class="ref-club-center"><article class="panel ref-roster"><div class="ref-box-title"><span>KADER – STARTELF</span><small>GESAMTEN KADER ANZEIGEN ›</small></div><div class="ref-squad-grid">${squadCards}</div></article><div class="ref-club-games"><article class="panel"><div class="ref-box-title"><span>LETZTE SPIELE</span></div>${recent.length?recent.map(m=>`<div class="ref-game-line"><small>${formatDateTime(m.scheduled_at)}</small><span>${esc(m.home_name)}</span><strong>${m.home_score}:${m.away_score}</strong><span>${esc(m.away_name)}</span></div>`).join(''):'<div class="ref-mini-empty">Noch keine Ergebnisse.</div>'}</article><article class="panel"><div class="ref-box-title"><span>NÄCHSTE SPIELE</span></div>${upcoming.length?upcoming.map(m=>`<div class="ref-game-line"><small>${formatDateTime(m.scheduled_at)}</small><span>${esc(m.home_name)}</span><strong>VS</strong><span>${esc(m.away_name)}</span></div>`).join(''):'<div class="ref-mini-empty">Noch keine Spiele geplant.</div>'}</article></div></section>
    <aside class="ref-club-side"><section class="panel ref-side-box"><div class="ref-box-title"><span>TOP SPIELER</span></div>${topPlayers.length?topPlayers.map((m,i)=>`<div class="ref-top-player"><b>${i+1}.</b><img src="${avatarFor(m)}"><span><strong>${esc(m.username)}</strong><small>${esc(m.position||'--')} | ${fmt(m.overall)} GES</small></span><em>${fmt(m.goals)} Tore</em></div>`).join(''):'<div class="ref-mini-empty">Noch keine Statistiken.</div>'}</section><section class="panel ref-side-box"><div class="ref-box-title"><span>CLUB ERFOLGE</span></div>${achievements.length?achievements.slice(0,4).map(a=>`<div class="ref-achievement-line">🏆 <span>${esc(a.title)}<small>${esc(a.subtitle||'')}</small></span></div>`).join(''):'<div class="ref-mini-empty">Noch keine Erfolge.</div>'}</section><section class="panel ref-side-box"><div class="ref-box-title"><span>TRANSFERMARKT</span></div>${transfers.length?transfers.slice(0,5).map(t=>`<div class="ref-transfer-line"><span>${esc(t.player)}</span><strong class="${t.to_club===c.name?'green':'red'}">${t.to_club===c.name?'ZUGANG':'ABGANG'}</strong></div>`).join(''):'<div class="ref-mini-empty">Keine Transfers.</div>'}</section></aside>
  </div></div>`;
}
...
function bindPage(p){
  ...
  if(p==='/admin') hydrateAdmin();
  if(p==='/manager') hydrateManager();
